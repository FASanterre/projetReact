...
from app.formulaires import FormulaireEditerProfil, FormulaireVide, FormulairePublication
...
def index():
    ...
    publications = current_user.liste_publications_dont_je_suis_partisan().paginate(page, app.config['PUBLICATION_PAR_PAGE'],False)
    ...
    return render_template('index.html', titre='Accueil', utilisateur=utilisateur, publications=publications.items, formulaire=formulaire, suivant=suivant, precedant=precedant) 

@app.route('/explorer')
@login_required
def explorer():
    page = request.args.get('page', 1, type=int)
    publications = Publication.query.order_by(Publication.horodatage.desc()).paginate(page, app.config['PUBLICATION_PAR_PAGE'],False)

    suivant = url_for('index', page=publications.next_num) \
        if publications.has_next else None
    precedant = url_for('index', page=publications.prev_num) \
        if publications.has_prev else None
    return render_template('index.html', titre='Explorer', publications=publications.items, suivant=suivant, precedant=precedant)
